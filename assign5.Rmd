...
Using the `unique()` function above, I can see that there are 9189 unique sequences. I will first filter by this, to cut down on the size of the data set. 
...
I now 9264 unique sequences. This is still more than 1 sequence per species, however, so I will remove/collapse the additional sequences for each species so that I only have 1 sequence per species, then join with the IUCN data set. 

```{r, results='hide'}
# This will simply keep the first entry for each unique entry in the species_name column. While this may not capture all sequence variation, it is an unbiased way to choose one individual to phylogenetically represent the species. 
...
194 observations is a far easier amount of data to work with than the original 965; however, this is still too many. I will take a random sample of 100 as a final reduction. 

```{r}
set.seed(12)
dtFishJoin100 <- sample_n(dtFishJoinNA, 100, replace = F)
...
# Sequence alignment
AlignedFishSeqs <- msa(FishSeqs)
AlignedFishSeqs
(msaCheckNames(AlignedFishSeqs))
...
plot(FishTree, main="Phylogenetic Tree of Cyprinid Sequences")
...
```{r, fig.show='hold', fig.cap = "Barplot of number of species per ICUN Red List classification. 1 box = 10 species, n = 100."}
...
ggplot(FishWaffle, aes(fill = redlistRank, values = n)) +
  geom_waffle(n_rows = 10, size = 0.33, colour = "white", flip = T) +
...
## Fig 2: Dendeogram or msa PrettyPrint
